...
for i in range(N):
    idx=upper_bound(pictures[i][0]-S) #현재 그림보다 S만큼 높이 낮은 그림보다 높이 높은 그림 중 가장 높이 낮은 그림의 인덱스 리턴받음
    dp.append(max(dp[i], dp[idx]+pictures[i][1])) #기존에 저장되어있던 값과 현재 그림의 가격+방금 구한 인덱스 그림의 총 가격 정보 더한 값을 비교해
